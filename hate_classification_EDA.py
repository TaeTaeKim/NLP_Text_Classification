...
from transformers import logging, get_linear_schedule_with_warmup,get_cosine_schedule_with_warmup
...
# config 파일 불러오기
config_path = os.path.join('config_hate_eda.json')
...
test_path = os.path.join(args.data_dir,'EDA_test.csv')
...
labels =  label_encoder.fit_transform(train_df['hate'])
...
def train(model, train_data, val_data, args, mode = 'train'):
    ...
    scheduler = get_cosine_schedule_with_warmup(optimizer, num_warmup_steps=int(total_steps * args.warmup_proportion), 
...
hate_dict = {0: 'hate', 1: 'none', 2: 'hate'}
...
original['hate'] = pred_hate
original.to_csv(f'./result/submission_{args.run}.csv',index=False)
